...
struct tx_signing_result
{
   ...
   xgt::protocol::public_key_type key;
   xgt::protocol::signature_type  sig;
};
...
int main(int argc, char** argv, char** envp)
{
   ...
   while( std::cin )
   {
      ...
      try
      {
         ...
         auto priv_key = xgt::utilities::wif_to_key( sreq.wif );

         if(priv_key)
         {
            sres.sig = priv_key->sign_compact( sres.sig_digest );
            sres.key = xgt::protocol::public_key_type( priv_key->get_public_key() );
