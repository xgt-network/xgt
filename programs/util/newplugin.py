...
target_link_libraries( {plugin_provider}_{plugin_name} xgtit_app xgtit_chain xgtit_protocol )
...
{{
   public:
      {plugin_name}_api( const xgt::app::api_context& ctx );
...
class {plugin_name}_plugin : public xgt::app::plugin
{{
   public:
      {plugin_name}_plugin( xgt::app::application* app );
...
"{plugin_name}_api.cpp" :
"""
#include <xgt/app/api_context.hpp>
#include <xgt/app/application.hpp>
...
{{
   public:
      {plugin_name}_api_impl( xgt::app::application& _app );

      std::shared_ptr< {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin > get_plugin();

      xgt::app::application& app;
}};

{plugin_name}_api_impl::{plugin_name}_api_impl( xgt::app::application& _app ) : app( _app )
{{}}
...
{plugin_name}_api::{plugin_name}_api( const xgt::app::api_context& ctx )
{{
...
{{
   public:
      {plugin_name}_plugin_impl( xgt::app::application& app );
      ...
      xgt::app::application& _app;
      boost::signals2::scoped_connection _applied_block_conn;
}};

{plugin_name}_plugin_impl::{plugin_name}_plugin_impl( xgt::app::application& app )
  : _app(app) {{}}
...
{plugin_name}_plugin::{plugin_name}_plugin( xgt::app::application* app )
   : plugin(app)
{{
...
XGT_DEFINE_PLUGIN( {plugin_name}, {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin )
""",
}
