...
#define WALLET_HISTORY_LENGTH_LIMIT  30
#define WALLET_HISTORY_TIME_LIMIT    30
...
using xgt::protocol::signed_block_header;
using xgt::protocol::signed_transaction;

using xgt::chain::operation_notification;
using xgt::chain::transaction_id_type;

using xgt::utilities::benchmark_dumper;
...
/** Represents an AH entry in mapped to wallet name.
 *  Holds additional informations, which are needed to simplify pruning process.
 *  All operations specific to given wallet, are next mapped to ID of given object.
 */
class wallet_history_info
{
public:
   int64_t        id = 0;
...
/// Pairs wallet_name storage type and the ID to make possible nonunique index definition over names.
typedef std::pair<wallet_name_type::Storage, size_t> wallet_name_storage_id_pair;
...
typedef PrimitiveTypeComparatorImpl<wallet_name_type::Storage> by_wallet_name_ComparatorImpl;
...
/// Compares wallet_history_info::id and rocksdb_operation_object::id pair
...
typedef PrimitiveTypeSlice< wallet_name_type::Storage > ah_info_by_name_slice_t;
...
const Comparator* by_wallet_name_Comparator()
{
   static by_wallet_name_ComparatorImpl c;
   return &c;
}
...
public:
   ...
   bool getAHInfo(const wallet_name_type& name, wallet_history_info* ahInfo) const
   {
   ...
   void putAHInfo(const wallet_name_type& name, const wallet_history_info& ahInfo)
   {
...
private:
   ...
   std::map<wallet_name_type, wallet_history_info> _ahInfoCache;
};
...
public:
   impl( wallet_history_plugin& self, const bpo::variables_map& options, const bfs::path& storagePath) :
      ...
      _mainDb.add_pre_reindex_handler([&]( const xgt::chain::reindex_notification& note ) -> void
         {
            on_pre_reindex( note );
         }, _self, 0);

      _mainDb.add_post_reindex_handler([&]( const xgt::chain::reindex_notification& note ) -> void
         {
            on_post_reindex( note );
         }, _self, 0);

      XGT_ADD_PLUGIN_INDEX(_mainDb, volatile_operation_index);
      }
   ...
   void on_pre_reindex( const xgt::chain::reindex_notification& note );
   void on_post_reindex( const xgt::chain::reindex_notification& note );

   /// Allows to start immediate data import (outside replay process).
   void importData(unsigned int blockLimit);

   void find_wallet_history_data(const wallet_name_type& name, uint64_t start, uint32_t limit,
...
private:
   ...
   void buildAccountHistoryRecord( const wallet_name_type& name, const rocksdb_operation_object& obj );
   void prunePotentiallyTooOldItems(wallet_history_info* ahInfo, const wallet_name_type& name,
   ...
   /** Returns true if given wallet is tracked.
    *  Depends on `"account-history-whitelist-ops"`, `account-history-blacklist-ops` option usage.
    *  Only some wallets can be chosen for tracking operation history.
    */
   bool isTrackedAccount(const wallet_name_type& name) const;

   /** Returns a collection of the wallets being impacted by given `op` and
    *  also tracked, because of passed options.
    *
    *  \see isTrackedAccount.
    */
   std::vector<wallet_name_type> getImpactedAccounts(const operation& op) const;
...
private:
   ...
   /// Total number of wallets (impacted by ops) excluded from processing because of filtering.
   ...
   wallet_name_range_index          _tracked_wallets;
...
inline bool wallet_history_plugin::impl::isTrackedAccount(const wallet_name_type& name) const
{
   ...
   /// Code below is based on original contents of wallet_history_plugin_impl::on_post_apply_operation
   auto itr = _tracked_wallets.lower_bound(name);
   ...
   if(itr != _tracked_wallets.begin() &&
      ((itr != _tracked_wallets.end() && itr->first != name ) || itr == _tracked_wallets.end()))
   {
      --itr;
   }

   bool inRange = itr != _tracked_wallets.end() && itr->first <= name && name <= itr->second;

   _excludedAccountCount += inRange;

   DIAGNOSTIC(
      if(inRange)
         ilog("Account: ${a} matched to defined wallet range: [${b},${e}]", ("a", name)("b", itr->first)("e", itr->second) );
      else
...
std::vector<wallet_name_type> wallet_history_plugin::impl::getImpactedAccounts(const operation& op) const
{
   ...
   std::vector<wallet_name_type> retVal;
...
void wallet_history_plugin::impl::storeOpFilteringParameters(const std::vector<std::string>& opList,
   flat_set<std::string>* storage) const
   {
      for(const auto& arg : opList)
      {
         ...
         {
            if( op.empty() == false )
               storage->insert( XGT_NAMESPACE_PREFIX + op );
         }
      }
   }
...
wallet_history_plugin::impl::ColumnDefinitions wallet_history_plugin::impl::prepareColumnDefinitions(bool addDefaultColumn)
{
   ...
   columnDefs.emplace_back("wallet_history_info_by_name", ColumnFamilyOptions());
   auto& byAccountNameColumn = columnDefs.back();
   byAccountNameColumn.options.comparator = by_wallet_name_Comparator();
...
void wallet_history_plugin::impl::buildAccountHistoryRecord( const wallet_name_type& name, const rocksdb_operation_object& obj )
{
   ...
   if(found)
   {
      ...
      if(_prune && count > WALLET_HISTORY_LENGTH_LIMIT &&
         ((obj.timestamp - ahInfo.oldestEntryTimestamp) > fc::days(WALLET_HISTORY_TIME_LIMIT)))
         {
   ...
   const fc::time_point_sec& now)
{
   ...
   auto ageLimit =  fc::days(WALLET_HISTORY_TIME_LIMIT);
   ...
   auto lookupUpperBound = std::make_pair(ahInfo->id + 1,
      ahInfo->newestEntryId - WALLET_HISTORY_LENGTH_LIMIT + 1);
    ...
    *  So just operations having a list position > WALLET_HISTORY_LENGTH_LIMIT and older that WALLET_HISTORY_TIME_LIMIT
    *  shall be removed.
    */
...
void wallet_history_plugin::impl::on_post_apply_operation(const operation_notification& n)
{
   ...
   if( impacted.empty() )
      return; // Ignore operations not impacting any wallet (according to original implementation)
