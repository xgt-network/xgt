...
using xgt::protocol::block_header;
...
public:
   ...
   virtual xgt::protocol::chain_id_type get_chain_id() const override;
...
xgt::protocol::chain_id_type p2p_plugin_impl::get_chain_id() const
{
...
void p2p_plugin::plugin_initialize(const boost::program_options::variables_map& options)
{
   ...
   my->user_agent = "Xgt Reference Implementation";
   ...
   if( options.count( "seed-node" ) || options.count( "p2p-seed-node" ) )
   {
      ...
      {
         // wlog( "p2p-seed-node ${h}", ("h", s) );
         auto s = options.at("p2p-seed-node").as<vector<string>>();
         if (s.size() == 1 && s[0] == "") {
           wlog( "no p2p-seed-node provided" );
         } else {
            for( auto& arg : s )
            {
               vector< string > addresses;
...
void p2p_plugin::broadcast_block( const xgt::protocol::signed_block& block )
{
