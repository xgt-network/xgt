...
{
public:
   transaction_status_impl() : _db( appbase::app().get_plugin< xgt::plugins::chain::chain_plugin >().db() ) {}
...
void transaction_status_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   try
   {
      ...
      my->actual_track_after_block = std::max< int64_t >( 0, int64_t( my->nominal_track_after_block ) - int64_t( XGT_MAX_TIME_UNTIL_EXPIRATION / XGT_BLOCK_INTERVAL ) );
      ...
      XGT_ADD_PLUGIN_INDEX(my->_db, transaction_status_index);
