...
template< typename OperationType=xgt::protocol::operation >
class evaluator
{
   public:
      virtual ~evaluator() {}
...
template< typename EvaluatorType, typename OperationType=xgt::protocol::operation >
...
#define XGT_DEFINE_EVALUATOR( X ) \
class X ## _evaluator : public xgt::chain::evaluator_impl< X ## _evaluator > \
{                                                                              \
   public:                                                                     \
      typedef X ## _operation operation_type;                                  \
                                                                               \
      X ## _evaluator( database& db )                                          \
         : xgt::chain::evaluator_impl< X ## _evaluator >( db )               \
      {}                                                                       \
                                                                               \
      void do_apply( const X ## _operation& o );                               \
};

#define XGT_DEFINE_ACTION_EVALUATOR( X, ACTION )                                       \
class X ## _evaluator : public xgt::chain::evaluator_impl< X ## _evaluator, ACTION > \
{                                                                                      \
   public:                                                                             \
      typedef X ## _action operation_type;                                             \
                                                                                       \
      X ## _evaluator( database& db )                                                  \
         : xgt::chain::evaluator_impl< X ## _evaluator, ACTION >( db )               \
      {}                                                                               \
                                                                                       \
      void do_apply( const X ## _action& o );                                          \
};

#define XGT_DEFINE_PLUGIN_EVALUATOR( PLUGIN, OPERATION, X )                               \
class X ## _evaluator : public xgt::chain::evaluator_impl< X ## _evaluator, OPERATION > \
{                                                                                         \
   public:                                                                                \
      typedef X ## _operation operation_type;                                             \
                                                                                          \
      X ## _evaluator( xgt::chain::database& db, PLUGIN* plugin )                       \
         : xgt::chain::evaluator_impl< X ## _evaluator, OPERATION >( db ),              \
           _plugin( plugin )                                                              \
      {}                                                                                  \
                                                                                          \
      void do_apply( const X ## _operation& o );                                          \
