 ...
 * the pool is full (XTT_MAX_NAI_POOL_COUNT) or we encounter the maximum
 * acceptable collisions (XTT_MAX_NAI_GENERATION_TRIES). If we hit the
...
void replenish_nai_pool( database& db )
{
   try
   {
       ...
       */
      if ( collisions_per_block < XTT_MAX_NAI_GENERATION_TRIES )
      {
...
static void refill_nais(
   ...
   nai_pool_object::pool_type& nais )
{
   while ( num_available_nais < XTT_MAX_NAI_POOL_COUNT )
   {
      ...
      for (;;)
      {
         ...
         // We must ensure the NAI is not an XTT, and it is not already contained within the NAI pool
         if ( !util::xtt::find_token( db, next_sym, true ) && std::find( std::begin( nais ), std::begin( nais ) + num_available_nais, next_sym ) != std::end( nais ) )
            break;
...
void remove_from_nai_pool( database &db, const asset_symbol_type& a )
{
   ...
   auto it = std::find( nais.begin(), end, asset_symbol_type::from_asset_num( a.get_stripped_precision_xtt_num() ) );
   if ( it != end )
   {
