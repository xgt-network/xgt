...
{
   FC_ASSERT( user_params.budget_per_time_unit >= XGT_RD_MIN_BUDGET );
   FC_ASSERT( user_params.budget_per_time_unit <= XGT_RD_MAX_BUDGET );
   FC_ASSERT( user_params.decay_per_time_unit >= XGT_RD_MIN_DECAY );
   FC_ASSERT( user_params.decay_per_time_unit <= XGT_RD_MAX_DECAY );
}
   ...
   rd_dynamics_params& dparams_out )
{
   ...
   //   XGT_RD_DECAY_DENOM_SHIFT.  The interface is designed to support other values of this
   ...
   //   code so that it sets decay_per_time_unit_denom_shift = XGT_RD_DECAY_DENOM_SHIFT,
   ...
   //
   FC_ASSERT( system_params.decay_per_time_unit_denom_shift == XGT_RD_DECAY_DENOM_SHIFT );
   ...
   static_assert( XGT_RD_MAX_BUDGET < (uint64_t(1) << (64-XGT_RD_DECAY_DENOM_SHIFT)),
      "Computation of temp could overflow here, set smaller XGT_RD_MAX_BUDGET" );
