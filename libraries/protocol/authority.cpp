...
void authority::add_authority( const wallet_name_type& k, weight_type w )
{
   wallet_auths[k] = w;
}
...
uint32_t authority::num_auths()const { return wallet_auths.size() + key_auths.size(); }

void authority::clear() { wallet_auths.clear(); key_auths.clear(); }

void authority::validate()const
{
   for( const auto& item : wallet_auths )
   {
      FC_ASSERT( is_valid_wallet_name( item.first ) );
   }
}
...
#if XGT_MIN_WALLET_NAME_LENGTH < 3
#error This is_valid_wallet_name implementation implicitly enforces minimum name length of 3.
#endif
...
//    if( len < XGT_MIN_WALLET_NAME_LENGTH )
//       return false;

//    if( len > XGT_MAX_WALLET_NAME_LENGTH )
//       return false;
