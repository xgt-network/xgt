...
bool sign_state::check_authority( string id )
{
   ...
   return check_authority_impl( get_money(id), 0, &account_auth_count );
}
...
bool sign_state::check_authority_impl( const authority& auth, uint32_t depth, uint32_t* account_auth_count )
{
   ...
   for( const auto& a : auth.wallet_auths )
   {
      if( approved_by.find(a.first) == approved_by.end() )
      {
         ...
         if( max_wallet_auths > 0 && *account_auth_count >= max_wallet_auths )
         {
            return false;
         }
         ...
         if( check_authority_impl( get_money( a.first ), depth + 1, account_auth_count ) )
         {
...
sign_state::sign_state(
   ...
   ) : get_money(a), available_keys(keys)
{
