...
using namespace xgt::plugins::rc;
...
BOOST_AUTO_TEST_CASE( rc_delegate_to_pool_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Test failure delegating to non-existent XTT" );
      ...
      BOOST_TEST_MESSAGE( "--- Test failure delegating to XTT in setup phase" );
      ...
      BOOST_TEST_MESSAGE( "--- Test failure delegating to XTT in setup_completed phase" );

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get< xtt_token_object, by_symbol >( alice_symbol ), [&]( xtt_token_object& xtt )
         {
            xtt.phase = xtt_phase::setup_completed;
         });
      });
      ...
      BOOST_TEST_MESSAGE( "--- Test failure delegating to XTT in ico phase" );

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get< xtt_token_object, by_symbol >( alice_symbol ), [&]( xtt_token_object& xtt )
         {
            xtt.phase = xtt_phase::ico;
         });
      });
      ...
      BOOST_TEST_MESSAGE( "--- Test failure delegating to XTT in launch_failed phase" );

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get< xtt_token_object, by_symbol >( alice_symbol ), [&]( xtt_token_object& xtt )
         {
            xtt.phase = xtt_phase::launch_failed;
         });
      });
      ...
      BOOST_TEST_MESSAGE( "--- Test success delegating to XTT in ico_completed phase" );

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get< xtt_token_object, by_symbol >( alice_symbol ), [&]( xtt_token_object& xtt )
         {
            xtt.phase = xtt_phase::ico_completed;
         });
      });
      ...
      BOOST_TEST_MESSAGE( "--- Test success delegating to XTT in setup_success phase" );
...
BOOST_AUTO_TEST_CASE( rc_set_slot_delegator )
{
   try
   {
      ...
      op.from_pool = XGT_TEMP_ACCOUNT;
      op.to_account = "dave";
      op.to_slot = -1;
      op.signer = "alice";
      BOOST_CHECK_THROW( op.validate(), fc::assert_exception );

      op.to_slot = XGT_RC_MAX_SLOTS;
      BOOST_CHECK_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( rc_delegate_drc_from_pool )
{
   try
   {
...

      {
         ...
         BOOST_REQUIRE( edge.drc_energybar.current_energy == op.drc_max_energy );
         BOOST_REQUIRE( edge.drc_energybar.last_update_time == db->head_block_time().sec_since_epoch() );
         BOOST_REQUIRE( edge.drc_max_energy == op.drc_max_energy );
      }

      generate_block();

      op.drc_max_energy = 50;
      custom_op.json = fc::json::to_string( rc_plugin_operation( op ) );
      tx.clear();
      tx.operations.push_back( custom_op );
      sign( tx, bob_private_key );
...
BOOST_AUTO_TEST_CASE( rc_drc_pool_consumption )
{
   try
   {
      ...
      using xgt::plugins::rc::detail::use_account_rcs;
      ...
      */
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         db.modify( db.get< rc_account_object, by_name >( "alice" ), [&]( rc_account_object& rca )
         {
            ...
            rca.rc_energybar.current_energy = 50;
            rca.rc_energybar.last_update_time = db.head_block_time().sec_since_epoch();
         ...
         db.create< rc_delegation_pool_object >( [&]( rc_delegation_pool_object& rc_pool )
         {
            ...
            rc_pool.rc_pool_energybar.current_energy = 100;
            rc_pool.rc_pool_energybar.last_update_time = db.head_block_time().sec_since_epoch();
            rc_pool.max_rc = 100;
         });
         ...
         db.create< rc_outdel_drc_edge_object >( [&]( rc_outdel_drc_edge_object& drc_edge )
         {
            ...
            drc_edge.drc_energybar.current_energy = 50;
            drc_edge.drc_energybar.last_update_time = db.head_block_time().sec_since_epoch();
            drc_edge.drc_max_energy = 50;
         });
         ...
         db.create< rc_delegation_pool_object >( [&]( rc_delegation_pool_object& rc_pool )
         {
            ...
            rc_pool.asset_symbol = VESTS_SYMBOL;
            rc_pool.rc_pool_energybar.current_energy = 25;
            rc_pool.rc_pool_energybar.last_update_time = db.head_block_time().sec_since_epoch();
            rc_pool.max_rc = 25;
         });
         ...
         {
            rca.max_rc_creation_adjustment.amount.value = 0;
            rca.rc_energybar.current_energy = 100;
            rca.rc_energybar.last_update_time = db.head_block_time().sec_since_epoch();
         ...
         db.create< rc_delegation_pool_object >( [&]( rc_delegation_pool_object& rc_pool )
         {
            ...
            rc_pool.asset_symbol = VESTS_SYMBOL;
            rc_pool.rc_pool_energybar.current_energy = 200;
            rc_pool.rc_pool_energybar.last_update_time = db.head_block_time().sec_since_epoch();
            rc_pool.max_rc = 200;
         });
         ...
         db.create< rc_outdel_drc_edge_object >( [&]( rc_outdel_drc_edge_object& drc_edge )
         {
            ...
            drc_edge.asset_symbol = VESTS_SYMBOL;
            drc_edge.drc_energybar.current_energy = 100;
            drc_edge.drc_energybar.last_update_time = db.head_block_time().sec_since_epoch();
            drc_edge.drc_max_energy = 100;
         });
      });
      ...
      BOOST_REQUIRE( poola_pool.rc_pool_energybar.current_energy == 50 );
      BOOST_REQUIRE( poola_drc_alice.drc_energybar.current_energy == 0 );
      BOOST_REQUIRE( poolb_pool.rc_pool_energybar.current_energy == 0 );
      BOOST_REQUIRE( poolb_drc_alice.drc_energybar.current_energy == 25 );
      BOOST_REQUIRE( alice_rc.rc_energybar.current_energy == 25 );

      use_account_rcs( *db, gpo, "bob", 50, skip, whitelist );
      BOOST_REQUIRE( poolc_pool.rc_pool_energybar.current_energy == 150 );
      BOOST_REQUIRE( poolc_drc_bob.drc_energybar.current_energy == 50 );
      BOOST_REQUIRE( bob_rc.rc_energybar.current_energy == 100 );

      db->set_producing( true );
      BOOST_CHECK_THROW( use_account_rcs( *db, gpo, "bob", 200, skip, whitelist ), xgt::chain::plugin_exception );
   }
   FC_LOG_AND_RETHROW()
}
