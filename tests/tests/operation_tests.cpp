...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      tx.signatures.clear();
      tx.operations.clear();
      op.fee = asset( db->get_account( XGT_INIT_MINER_NAME ).balance.amount + 1, XGT_SYMBOL );
      op.new_account_name = "bob";
      tx.operations.push_back( op );
      sign( tx, init_account_priv_key );
      ...
      fund( XGT_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );
      vest( XGT_INIT_MINER_NAME, XGT_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );

      BOOST_TEST_MESSAGE( "--- Test account creation with temp account does not set recovery account" );
      op.creator = XGT_TEMP_ACCOUNT;
...
BOOST_AUTO_TEST_CASE( account_update_authorities )
{
   try
   {
      ...
      XGT_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_active_auth );
      ...
      sign( tx, alice_private_key );
      XGT_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when containing duplicate signatures" );
      tx.signatures.clear();
      sign( tx, active_key );
      sign( tx, active_key );
      XGT_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_duplicate_sig );
      ...
      tx.operations.push_back( op );
      sign( tx, active_key );
      XGT_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_owner_auth );
...
BOOST_AUTO_TEST_CASE( comment_authorities )
{
   try
   {
      ...
      ACTORS( (alice)(bob) );
      generate_blocks( 60 / XGT_BLOCK_INTERVAL );
      ...
      XGT_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_posting_auth );
      ...
      tx.signatures.clear();
      sign( tx, bob_private_key );
      XGT_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_posting_auth );
...
BOOST_AUTO_TEST_CASE( comment_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.cashout_time == fc::time_point_sec( db->head_block_time() + fc::seconds( XGT_CASHOUT_WINDOW_SECONDS ) ) );
      ...
      BOOST_REQUIRE( sam_comment.cashout_time == sam_comment.created + XGT_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( sam_comment.root_comment == alice_comment.id );
      validate_database();

      generate_blocks( 60 * 5 / XGT_BLOCK_INTERVAL + 1 );
      ...
      {
         o.total_reward_shares2 = xgt::chain::util::evaluate_reward_curve( 10 );
      });
      ...
      BOOST_REQUIRE( mod_sam_comment.cashout_time == mod_sam_comment.created + XGT_CASHOUT_WINDOW_SECONDS );
      validate_database();
      ...
      generate_blocks( 60 * 5 / XGT_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_delete_apply )
{
   try
   {
      ...
      vest( XGT_INIT_MINER_NAME, "alice", ASSET( "1000.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + XGT_MIN_TRANSACTION_EXPIRATION_LIMIT );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
      ...
      generate_blocks( XGT_MIN_ROOT_COMMENT_INTERVAL.to_seconds() / XGT_BLOCK_INTERVAL );
      ...
      generate_blocks( XGT_CASHOUT_WINDOW_SECONDS / XGT_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
   try
   {
...

      {
         ...
         auto old_energy = alice.voting_energybar.current_energy;
         ...
         int64_t max_vote_denom = ( db->get_dynamic_global_properties().target_votes_per_period * XGT_VOTING_ENERGY_REGENERATION_SECONDS ) / (60*60*24);

         BOOST_REQUIRE( alice.last_vote_time == db->head_block_time() );
         BOOST_REQUIRE( alice_comment.net_rshares.value == ( old_energy - alice.voting_energybar.current_energy ) - XGT_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( alice_comment.cashout_time == alice_comment.created + XGT_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr->rshares == ( old_energy - alice.voting_energybar.current_energy ) - XGT_VOTE_DUST_THRESHOLD );
         ...
         util::energybar old_energybar = db->get_account( "alice" ).voting_energybar;
         util::energybar_params params( util::get_effective_vesting_shares( db->get_account( "alice" ) ), XGT_VOTING_ENERGY_REGENERATION_SECONDS );
         ...
         BOOST_REQUIRE( bob_comment.net_rshares.value == ( old_energybar.current_energy - db->get_account( "alice" ).voting_energybar.current_energy ) - XGT_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + XGT_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         old_energy = db->get_account( "bob" ).voting_energybar.current_energy;
         ...
         BOOST_REQUIRE( new_alice_comment.net_rshares.value == old_abs_rshares + ( old_energy - new_bob.voting_energybar.current_energy ) - XGT_VOTE_DUST_THRESHOLD );
         ...
         old_energybar = db->get_account( "sam" ).voting_energybar;
         params.max_energy = util::get_effective_vesting_shares( db->get_account( "sam" ) );
         old_energybar.regenerate_energy( params, db->head_block_time() );

         op.weight = -1 * XGT_100_PERCENT / 2;
         op.voter = "sam";
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         ...
         util::energybar_params downvote_params( util::get_effective_vesting_shares( db->get_account( "alice" ) ) / 4, XGT_VOTING_ENERGY_REGENERATION_SECONDS );
         old_downvote_energybar.regenerate_energy( downvote_params, db->head_block_time() );
         int64_t sam_weight = old_downvote_energybar.current_energy - db->get_account( "sam" ).downvote_energybar.current_energy - XGT_VOTE_DUST_THRESHOLD;
         ...
         int64_t regenerated_power = (XGT_100_PERCENT * ( db->head_block_time() - db->get_account( "alice").last_vote_time ).to_seconds() ) / XGT_VOTING_ENERGY_REGENERATION_SECONDS;
         ...
         int64_t new_rshares = ( ( fc::uint128_t( db->get_account( "alice" ).vesting_shares.amount.value ) * used_power ) / XGT_100_PERCENT ).to_uint64() - XGT_VOTE_DUST_THRESHOLD;
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( ( XGT_1_PERCENT * 25 * ( get_voting_power( new_alice ) ) / XGT_100_PERCENT ) + max_vote_denom - 1 ) / max_vote_denom;
         ...
         new_rshares = ( ( uint128_t( old_energybar.current_energy ) * 25 * XGT_1_PERCENT + votes_per_period ) / ( votes_per_period * XGT_100_PERCENT ) ).to_int64() - XGT_VOTE_DUST_THRESHOLD;

         op.voter = "alice";
         op.weight = XGT_1_PERCENT * 25;
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         tx.signatures.clear();
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( uint64_t( XGT_1_PERCENT ) * 75 * uint64_t( alice_voting_power ) ) / XGT_100_PERCENT;
         ...
         downvote_params.max_energy = util::get_effective_vesting_shares( db->get_account( "alice" ) ) / 4;
         old_downvote_energybar.regenerate_energy( downvote_params, db->head_block_time() );

         op.weight = XGT_1_PERCENT * -75;
         tx.operations.clear();
         tx.signatures.clear();
         tx.operations.push_back( op );
         sign( tx, alice_private_key );
         ...
         new_rshares = old_downvote_energybar.current_energy - db->get_account( "alice" ).downvote_energybar.current_energy - XGT_VOTE_DUST_THRESHOLD;
         ...
         {
            const auto& alice = db->get_account( "alice" );
            old_downvote_energybar.current_energy = alice.downvote_energybar.current_energy;
            old_downvote_energybar.last_update_time = alice.downvote_energybar.last_update_time;
            old_energybar.current_energy = alice.voting_energybar.current_energy;
            old_energybar.last_update_time = alice.voting_energybar.last_update_time;
            old_energybar.regenerate_energy( params, db->head_block_time() );
            old_downvote_energybar.regenerate_energy( params, db->head_block_time() );
            alice_weight = old_energybar.current_energy;
...

         {
            const auto& bob_comment = db->get_comment( "bob", string( "foo" ) );
            const auto& alice = db->get_account( "alice" );
            BOOST_REQUIRE( bob_comment.net_rshares == old_net_rshares - alice_weight + XGT_VOTE_DUST_THRESHOLD );
            BOOST_REQUIRE( bob_comment.abs_rshares == old_abs_rshares + alice_weight - XGT_VOTE_DUST_THRESHOLD );
            BOOST_REQUIRE( alice_bob_vote->rshares == -1 * ( alice_weight - XGT_VOTE_DUST_THRESHOLD ) );
            BOOST_REQUIRE( alice_bob_vote->last_update == db->head_block_time() );
            BOOST_REQUIRE( alice.downvote_energybar.current_energy == 0 );
            BOOST_REQUIRE( alice.voting_energybar.current_energy == old_energybar.current_energy - alice_weight + old_downvote_energybar.current_energy );
            validate_database();
         }

         BOOST_TEST_MESSAGE( "--- Test reduced effectiveness when increasing rshares within lockout period" );

         generate_blocks( fc::time_point_sec( ( new_bob_comment.cashout_time - XGT_UPVOTE_LOCKOUT_HF17 ).sec_since_epoch() + XGT_BLOCK_INTERVAL ), true );
         ...
         new_rshares = old_energybar.current_energy - db->get_account( "dave" ).voting_energybar.current_energy - XGT_VOTE_DUST_THRESHOLD;
         ...
         BOOST_TEST_MESSAGE( "--- Test energy charge when increasing vote" );
         {
            ...
            auto old_rshares = db->get< comment_vote_object, by_voter_comment_symbol >( boost::make_tuple( sam.id, db->get_comment( op.author, op.permlink ).id, XGT_SYMBOL ) ).rshares;
            ...
            new_rshares = ( ( uint128_t( old_energybar.current_energy ) * op.weight + votes_per_period - 1 ) / ( votes_per_period * XGT_100_PERCENT ) ).to_int64() - XGT_VOTE_DUST_THRESHOLD;
         ...
         BOOST_TEST_MESSAGE( "--- Test energy charge when changing to downvote" );
         {
            ...
            new_rshares = ( ( uint128_t( old_energybar.current_energy ) * 50 * XGT_1_PERCENT + votes_per_period - 1 ) / ( votes_per_period * XGT_100_PERCENT ) ).to_int64();

            BOOST_REQUIRE( sam.voting_energybar.current_energy == old_energybar.current_energy );
            BOOST_REQUIRE( sam.downvote_energybar.current_energy == old_downvote_energybar.current_energy - new_rshares );
         }
         ...
         BOOST_TEST_MESSAGE( "--- Test energy charge when increasing downvote" );
         {
            ...
            new_rshares = ( ( uint128_t( old_energybar.current_energy ) * XGT_100_PERCENT + votes_per_period - 1 ) / ( votes_per_period * XGT_100_PERCENT ) ).to_int64();
            auto delta = new_rshares - ( std::abs( old_rshares ) + XGT_VOTE_DUST_THRESHOLD );
...
BOOST_AUTO_TEST_CASE( transfer_validate )
{
   try
   {
      ...
      op.to = "bob";
      op.memo = "Memo";
      op.amount = asset( 100, XGT_SYMBOL );
      op.validate();
...
BOOST_AUTO_TEST_CASE( transfer_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Test failure transfering XGT to xgt.dao" );
      op.from = "bob";
      op.to = XGT_TREASURY_ACCOUNT;
      op.amount = ASSET( "1.000 TESTS" );
      tx.signatures.clear();
      tx.operations.clear();
      tx.operations.push_back( op );
      sign( tx, bob_private_key );
      XGT_REQUIRE_THROW( db->push_transaction( tx ), fc::exception );

      BOOST_REQUIRE( db->get_account( "bob" ).balance == ASSET( "10.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( XGT_TREASURY_ACCOUNT ).balance == ASSET( "0.000 TESTS" ) );
      validate_database();

      BOOST_TEST_MESSAGE( "--- Test transfering SBD to xgt.dao" );
      auto treasury_sbd_balance = db->get_account( XGT_TREASURY_ACCOUNT ).sbd_balance;
      ...
      BOOST_REQUIRE( db->get_account( XGT_TREASURY_ACCOUNT ).sbd_balance == treasury_sbd_balance + ASSET( "1.000 TBD" ) );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( transfer_to_vesting_apply )
{
   try
   {
      ...
      auto vests = asset( gpo.total_vesting_fund_xgt.amount, XGT_SYMBOL );
      ...
      BOOST_REQUIRE( gpo.total_vesting_fund_xgt.amount.value == vests.amount.value );
      BOOST_REQUIRE( gpo.total_vesting_shares.amount.value == shares.amount.value );
      validate_database();

      op.to = "bob";
      op.amount = asset( 2000, XGT_SYMBOL );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + XGT_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_validate )
{
   ...
   op.vesting_shares = asset( 10, XGT_SYMBOL );
   BOOST_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( XGT_VESTING_WITHDRAW_INTERVALS * 2 ) ).value + 1 );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + XGT_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      BOOST_REQUIRE( alice.vesting_shares.amount.value == old_vesting_shares.amount.value );
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( XGT_VESTING_WITHDRAW_INTERVALS * 3 ) ).value + 1 );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + XGT_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         {
            gpo.current_supply += wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" ) - gpo.total_vesting_fund_xgt;
            gpo.total_vesting_fund_xgt = wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" );
         });
...
BOOST_AUTO_TEST_CASE( witness_update_apply )
{
   try
   {
      ...
      op.fee = ASSET( "1.000 TESTS" );
      op.block_signing_key = signing_key.get_public_key();
      op.props.account_creation_fee = legacy_xgt_asset::from_asset( asset(XGT_MIN_ACCOUNT_CREATION_FEE + 10, XGT_SYMBOL) );
      op.props.maximum_block_size = XGT_MIN_BLOCK_SIZE_LIMIT + 100;
...
BOOST_AUTO_TEST_CASE( account_witness_proxy_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.proxy == XGT_PROXY_TO_SELF_ACCOUNT );
      validate_database();
      ...
      BOOST_REQUIRE( sam.proxy == XGT_PROXY_TO_SELF_ACCOUNT );
      validate_database();
      ...
      BOOST_REQUIRE( dave.proxy == XGT_PROXY_TO_SELF_ACCOUNT );
      validate_database();
      ...
      tx.operations.clear();
      tx.signatures.clear();
      op.proxy = XGT_PROXY_TO_SELF_ACCOUNT;
      op.account = "bob";
      tx.operations.push_back( op );
      sign( tx, bob_private_key );
      ...
      BOOST_REQUIRE( alice.proxy == "sam" );
      BOOST_REQUIRE( bob.proxy == XGT_PROXY_TO_SELF_ACCOUNT );
      BOOST_REQUIRE( sam.proxy == "dave" );
      BOOST_REQUIRE( dave.proxy == XGT_PROXY_TO_SELF_ACCOUNT );
      validate_database();
      ...
      vote.account= "bob";
      vote.witness = XGT_INIT_MINER_NAME;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( vote );
      sign( tx, bob_private_key );
      ...
      BOOST_REQUIRE( db->get_witness( XGT_INIT_MINER_NAME ).votes == ( alice.vesting_shares + bob.vesting_shares ).amount );
      validate_database();
      ...
      BOOST_REQUIRE( db->get_witness( XGT_INIT_MINER_NAME ).votes == bob.vesting_shares.amount );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( custom_json_rate_limit )
{
   try
   {
      ...
      XGT_REQUIRE_THROW( db->push_transaction( tx, 0 ), plugin_exception );
...
BOOST_AUTO_TEST_CASE( feed_publish_apply )
{
   try
   {
      ...
      feed_publish_operation op;
      op.publisher = "alice";
      op.exchange_rate = price( ASSET( "1.000 TBD" ), ASSET( "1000.000 TESTS" ) ); // 1000 XGT : 1 SBD
...
BOOST_AUTO_TEST_CASE( convert_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( convert_request->conversion_date == db->head_block_time() + XGT_CONVERSION_DELAY );
...
BOOST_AUTO_TEST_CASE( fixture_convert_checks_balance )
{
   ...
   try
   {
      ...
      fund( "dany", 5000 );
      XGT_REQUIRE_THROW( convert( "dany", ASSET( "5000.000 TESTS" ) ), fc::exception );
   }
   FC_LOG_AND_RETHROW()
...
//      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( SBD_SYMBOL, XGT_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( limit_order_create2_apply )
{
   try
   {
      ...
      /// First check validation on price constructor level:
      {
        price broken_price;
        /// Invalid base value
        XGT_REQUIRE_THROW(broken_price=price(ASSET("0.000 TESTS"), ASSET("1.000 TBD")),
          fc::exception);
        /// Invalid quote value
        XGT_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TBD")),
          fc::exception);
        /// Invalid symbol (same in base & quote)
        XGT_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TESTS")),
          fc::exception);
      }
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
   try
   {
      ...
      vest( XGT_INIT_MINER_NAME, "bob", asset( 1000, XGT_SYMBOL ) );
      ...
      generate_blocks( db->head_block_time() + XGT_OWNER_UPDATE_LIMIT );
      ...
      generate_blocks( db->head_block_time() + XGT_OWNER_UPDATE_LIMIT + fc::seconds( XGT_BLOCK_INTERVAL ) );
      ...
      BOOST_REQUIRE( req_itr->expires == db->head_block_time() + XGT_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD );
      auto expires = req_itr->expires;
      ++req_itr;
      BOOST_REQUIRE( req_itr == request_idx.end() );

      generate_blocks( time_point_sec( expires - XGT_BLOCK_INTERVAL ), true );
      ...
      generate_blocks( db->head_block_time() + ( XGT_OWNER_AUTH_RECOVERY_PERIOD - XGT_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD ) );
      generate_block();
...
BOOST_AUTO_TEST_CASE( change_recovery_account )
{
   try
   {
      ...
      XGT_REQUIRE_THROW( change_recovery_account("alice", "nobody"), fc::exception );
      XGT_REQUIRE_THROW( change_recovery_account("haxer", "sam"   ), fc::exception );
      XGT_REQUIRE_THROW( change_recovery_account("haxer", "nobody"), fc::exception );
      ...
      generate_blocks( db->head_block_time() + XGT_OWNER_AUTH_RECOVERY_PERIOD - fc::seconds( XGT_BLOCK_INTERVAL ), true );
      // cannot request account recovery until recovery account is approved
      XGT_REQUIRE_THROW( request_account_recovery( "sam", sam_private_key, "alice", alice_pub1 ), fc::exception );
      generate_blocks(1);
      // cannot finish account recovery until requested
      XGT_REQUIRE_THROW( recover_account( "alice", alice_priv1, alice_private_key ), fc::exception );
      // do the request
...
BOOST_AUTO_TEST_CASE( escrow_transfer_validate )
{
   try
   {
      ...
      op.xgt_amount = ASSET( "1.000 TESTS" );
      op.escrow_id = 0;
      op.agent = "sam";
      op.fee = ASSET( "0.100 TESTS" );
      op.json_meta = "";
      ...
      op.sbd_amount.symbol = XGT_SYMBOL;
      XGT_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when xgt symbol != XGT" );
      op.sbd_amount.symbol = SBD_SYMBOL;
      op.xgt_amount.symbol = SBD_SYMBOL;
      XGT_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when fee symbol != SBD and fee symbol != XGT" );
      op.xgt_amount.symbol = XGT_SYMBOL;
      op.fee.symbol = VESTS_SYMBOL;
      XGT_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when sbd == 0 and xgt == 0" );
      op.fee.symbol = XGT_SYMBOL;
      op.sbd_amount.amount = 0;
      op.xgt_amount.amount = 0;
      XGT_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when sbd < 0" );
      op.sbd_amount.amount = -100;
      op.xgt_amount.amount = 1000;
      XGT_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when xgt < 0" );
      op.sbd_amount.amount = 1000;
      op.xgt_amount.amount = -100;
      XGT_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_transfer_apply )
{
   try
   {
      ...
      auto alice_xgt_balance = alice.balance - op.xgt_amount - op.fee;
      auto alice_sbd_balance = alice.sbd_balance - op.sbd_amount;
      auto bob_xgt_balance = bob.balance;
      auto bob_sbd_balance = bob.sbd_balance;
      auto sam_xgt_balance = sam.balance;
      ...
      BOOST_REQUIRE( escrow.xgt_balance == op.xgt_amount );
      ...
      BOOST_REQUIRE( alice.balance == alice_xgt_balance );
      BOOST_REQUIRE( alice.sbd_balance == alice_sbd_balance );
      BOOST_REQUIRE( bob.balance == bob_xgt_balance );
      BOOST_REQUIRE( bob.sbd_balance == bob_sbd_balance );
      BOOST_REQUIRE( sam.balance == sam_xgt_balance );
...
BOOST_AUTO_TEST_CASE( escrow_approve_apply )
{
   try
   {
      ...
      et_op.to = "bob";
      et_op.agent = "sam";
      et_op.xgt_amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_REQUIRE( escrow.xgt_balance == ASSET( "1.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + XGT_BLOCK_INTERVAL );
      sign( tx, bob_private_key );
      XGT_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      ...
      XGT_REQUIRE_THROW( db->get_escrow( op.from, op.escrow_id ), fc::exception );
      BOOST_REQUIRE( alice.balance == ASSET( "10.000 TESTS" ) );
      validate_database();
      ...
      generate_blocks( et_op.ratification_deadline + XGT_BLOCK_INTERVAL, true );
...
BOOST_AUTO_TEST_CASE( escrow_dispute_apply )
{
   try
   {
      ...
      et_op.xgt_amount = ASSET( "1.000 TESTS" );
      et_op.fee = ASSET( "0.100 TESTS" );
      et_op.ratification_deadline = db->head_block_time() + XGT_BLOCK_INTERVAL;
      et_op.escrow_expiration = db->head_block_time() + 2 * XGT_BLOCK_INTERVAL;
      ...
      BOOST_REQUIRE( escrow.xgt_balance == et_op.xgt_amount );
...
BOOST_AUTO_TEST_CASE( escrow_release_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when xgt == 0 and sbd == 0" );
      op.sbd_amount.amount = 0;
      XGT_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when xgt is not xgt symbol" );
      op.sbd_amount.symbol = SBD_SYMBOL;
      op.xgt_amount = ASSET( "1.000 TBD" );
      XGT_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_release_apply )
{
   try
   {
      ...
      op.xgt_amount = ASSET( "0.100 TESTS" );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).xgt_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).xgt_balance == ASSET( "0.800 TESTS" ) );
      ...
      BOOST_TEST_MESSAGE( "--- failure when releasing less xgt than available" );
      op.xgt_amount = ASSET( "0.000 TESTS" );
      op.sbd_amount = ASSET( "1.000 TBD" );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).xgt_balance == ASSET( "0.700 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).xgt_balance == ASSET( "0.600 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).xgt_balance == ASSET( "0.500 TESTS" ) );


      BOOST_TEST_MESSAGE( "--- success deleting escrow when balances are both zero" );
      tx.clear();
      op.xgt_amount = ASSET( "0.500 TESTS" );
      tx.operations.push_back( op );
      sign( tx, sam_private_key );
      db->push_transaction( tx, 0 );

      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "9.700 TESTS" ) );
      XGT_REQUIRE_THROW( db->get_escrow( et_op.from, et_op.escrow_id ), fc::exception );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).xgt_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).xgt_balance == ASSET( "0.800 TESTS" ) );
      ...
      tx.clear();
      op.xgt_amount = ASSET( "0.600 TESTS" );
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "success when amount is XGT" );
      op.amount = ASSET( "1.000 TESTS" );
      op.validate();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Failure when transferring to xgt.dao" );
      ...
      BOOST_TEST_MESSAGE( "--- success transferring XGT to self" );
      op.to = "alice";
      op.amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_TEST_MESSAGE( "--- success transferring XGT to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
...
BOOST_AUTO_TEST_CASE( transfer_from_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Failure withdrawing TESTS to xgt.dao" );
      ...
      BOOST_TEST_MESSAGE( "--- Success withdrawing TBD to xgt.dao" );
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing XGT to self" );
      op.to = "alice";
      op.amount = ASSET( "1.000 TESTS" );
      op.request_id++;
      ...
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).complete == db->head_block_time() + XGT_SAVINGS_WITHDRAW_TIME );
      validate_database();
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing XGT to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
      op.request_id = 3;
      ...
      generate_blocks( db->head_block_time() + XGT_SAVINGS_WITHDRAW_TIME - fc::seconds( XGT_BLOCK_INTERVAL ), true );
      ...
      for( int i = 0; i < XGT_SAVINGS_WITHDRAW_REQUEST_LIMIT; i++ )
      {
         op.request_id = i;
         tx.clear();
         tx.operations.push_back( op );
      ...
      op.request_id = XGT_SAVINGS_WITHDRAW_REQUEST_LIMIT;
      tx.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      XGT_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == XGT_SAVINGS_WITHDRAW_REQUEST_LIMIT );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( itr->effective_date == db->head_block_time() + XGT_OWNER_AUTH_RECOVERY_PERIOD );
      ...
      db->get< comment_vote_object, by_comment_voter_symbol >( boost::make_tuple( db->get_comment( "alice", string( "test" ) ).id, db->get_account( "alice" ).id, XGT_SYMBOL )  );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_validate )
{
   try
   {
      ...
      op.account = "alice";
      op.reward_xgt = ASSET( "0.000 TESTS" );
      ...
      op.reward_xgt.amount = 1000;
      op.validate();

      op.reward_xgt.amount = 0;
      ...
      BOOST_TEST_MESSAGE( "Testing wrong XGT symbol" );
      op.reward_xgt = ASSET( "1.000 TBD" );
      XGT_REQUIRE_THROW( op.validate(), fc::assert_exception );


      BOOST_TEST_MESSAGE( "Testing wrong SBD symbol" );
      op.reward_xgt = ASSET( "1.000 TESTS" );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Attempting to claim more XGT than exists in the reward balance." );

      claim_reward_balance_operation op;
      signed_transaction tx;

      op.account = "alice";
      op.reward_xgt = ASSET( "20.000 TESTS" );
      op.reward_sbd = ASSET( "0.000 TBD" );
      op.reward_vests = ASSET( "0.000000 VESTS" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_xgt + op.reward_xgt );
      ...
      op.reward_xgt = ASSET( "10.000 TESTS" );
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_authorities )
{
   try
   {
      ...
      signed_transaction tx;
      ACTORS( (alice)(bob) )
      vest( XGT_INIT_MINER_NAME, "alice", ASSET( "10000.000 TESTS" ) );
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_apply )
{
   try
   {
      ...
      util::energybar old_downvote_energybar = db->get_account( "alice" ).downvote_energybar;
      params.max_energy = util::get_effective_vesting_shares( db->get_account( "alice" ) ) / 4;
      old_downvote_energybar.regenerate_energy( params, db->head_block_time() );

      util::energybar old_bob_energybar = db->get_account( "bob" ).voting_energybar;
      params.max_energy = util::get_effective_vesting_shares( db->get_account( "bob" ) );
      old_bob_energybar.regenerate_energy( params, db->head_block_time() );

      util::energybar old_bob_downvote_energybar = db->get_account( "bob" ).downvote_energybar;
      params.max_energy = util::get_effective_vesting_shares( db->get_account( "bob" ) ) / 4;
      old_bob_downvote_energybar.regenerate_energy( params, db->head_block_time() );
      ...
      BOOST_REQUIRE( alice_acc.voting_energybar.current_energy == old_energybar.current_energy - op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice_acc.downvote_energybar.current_energy == old_downvote_energybar.current_energy - op.vesting_shares.amount.value / 4 );
      BOOST_REQUIRE( bob_acc.received_vesting_shares == ASSET( "10000000.000000 VESTS" ) );
      BOOST_REQUIRE( bob_acc.voting_energybar.current_energy == old_bob_energybar.current_energy + op.vesting_shares.amount.value );
      BOOST_REQUIRE( bob_acc.downvote_energybar.current_energy == old_bob_downvote_energybar.current_energy + op.vesting_shares.amount.value / 4 );

      BOOST_TEST_MESSAGE( "--- Test that the delegation object is correct. " );
      auto delegation = db->find< vesting_delegation_object, by_delegation >( boost::make_tuple( op.delegator, op.delegatee, XGT_SYMBOL ) );
      ...
      old_bob_energybar = db->get_account( "bob" ).voting_energybar;
      params.max_energy = util::get_effective_vesting_shares( db->get_account( "bob" ) );
      old_bob_energybar.regenerate_energy( params, db->head_block_time() );

      old_bob_downvote_energybar = db->get_account( "bob" ).downvote_energybar;
      params.max_energy = util::get_effective_vesting_shares( db->get_account( "bob" ) ) / 4;
      old_bob_downvote_energybar.regenerate_energy( params, db->head_block_time() );
      ...
      idump( (alice_acc.voting_energybar)(old_energybar)(delta) );
      ...
      BOOST_REQUIRE( alice_acc.voting_energybar.current_energy == old_energybar.current_energy - delta );
      BOOST_REQUIRE( alice_acc.downvote_energybar.current_energy == old_downvote_energybar.current_energy - delta / 4 );
      BOOST_REQUIRE( bob_acc.received_vesting_shares == ASSET( "20000000.000000 VESTS"));
      BOOST_REQUIRE( bob_acc.voting_energybar.current_energy == old_bob_energybar.current_energy + delta );
      BOOST_REQUIRE( bob_acc.downvote_energybar.current_energy == old_bob_downvote_energybar.current_energy + delta / 4 );
      ...
      BOOST_REQUIRE( alice_comment.net_rshares.value == old_energybar.current_energy - db->get_account( "bob" ).voting_energybar.current_energy - XGT_VOTE_DUST_THRESHOLD );
      BOOST_REQUIRE( itr->rshares == old_energybar.current_energy - db->get_account( "bob" ).voting_energybar.current_energy - XGT_VOTE_DUST_THRESHOLD );

      generate_block();
      ACTORS( (sam)(dave) )
      generate_block();

      vest( XGT_INIT_MINER_NAME, "sam", ASSET( "1000.000 TESTS" ) );
      ...
      BOOST_TEST_MESSAGE( "--- Testing failure delegating when there is not enough energy" );
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         {
            a.voting_energybar.current_energy = a.downvote_energybar.current_energy * 3 / 4;
            a.voting_energybar.last_update_time = db.head_block_time().sec_since_epoch();
         });
      });
      ...
      BOOST_TEST_MESSAGE( "--- Testing failure delegating when there is not enough downvote energy" );
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( "sam" ), [&]( account_object& a )
         {
            a.voting_energybar.current_energy = util::get_effective_vesting_shares( a );
            a.voting_energybar.last_update_time = db.head_block_time().sec_since_epoch();
            a.downvote_energybar.current_energy = a.downvote_energybar.current_energy * 3 / 4;
            a.downvote_energybar.last_update_time = db.head_block_time().sec_since_epoch();
         });
      });
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( "sam" ), [&]( account_object& a )
         {
            a.downvote_energybar.current_energy = util::get_effective_vesting_shares( a ) / 4;
            a.downvote_energybar.last_update_time = db.head_block_time().sec_since_epoch();
         });
      });
      ...
      util::energybar old_sam_energybar = db->get_account( "sam" ).voting_energybar;
      util::energybar old_sam_downvote_energybar = db->get_account( "sam" ).downvote_energybar;
      util::energybar old_dave_energybar = db->get_account( "dave" ).voting_energybar;
      util::energybar old_dave_downvote_energybar = db->get_account( "dave" ).downvote_energybar;

      util::energybar_params sam_params( util::get_effective_vesting_shares( db->get_account( "sam" ) ), XGT_VOTING_ENERGY_REGENERATION_SECONDS );
      util::energybar_params dave_params( util::get_effective_vesting_shares( db->get_account( "dave" ) ), XGT_VOTING_ENERGY_REGENERATION_SECONDS );
      ...
      delegation = db->find< vesting_delegation_object, by_delegation >( boost::make_tuple( op.delegator, op.delegatee, XGT_SYMBOL ) );
      BOOST_REQUIRE( delegation == nullptr );

      old_sam_energybar.regenerate_energy( sam_params, db->head_block_time() );
      sam_params.max_energy /= 4;
      old_sam_downvote_energybar.regenerate_energy( sam_params, db->head_block_time() );

      old_dave_energybar.regenerate_energy( dave_params, db->head_block_time() );
      dave_params.max_energy /= 4;
      old_dave_downvote_energybar.regenerate_energy( dave_params, db->head_block_time() );

      BOOST_REQUIRE( db->get_account( "sam" ).voting_energybar.current_energy == old_sam_energybar.current_energy );
      BOOST_REQUIRE( db->get_account( "sam" ).downvote_energybar.current_energy == old_sam_downvote_energybar.current_energy );
      BOOST_REQUIRE( db->get_account( "dave" ).voting_energybar.current_energy == old_dave_energybar.current_energy - sam_vest.amount.value );
      BOOST_REQUIRE( db->get_account( "dave" ).downvote_energybar.current_energy == old_dave_downvote_energybar.current_energy - sam_vest.amount.value / 4 );

      old_sam_energybar = db->get_account( "sam" ).voting_energybar;
      old_sam_downvote_energybar = db->get_account( "sam" ).downvote_energybar;
      old_dave_energybar = db->get_account( "dave" ).voting_energybar;
      old_dave_downvote_energybar = db->get_account( "dave" ).downvote_energybar;

      sam_params.max_energy = util::get_effective_vesting_shares( db->get_account( "sam" ) );
      dave_params.max_energy = util::get_effective_vesting_shares( db->get_account( "dave" ) );
      ...
      BOOST_REQUIRE( db->get_account( "sam" ).voting_energybar.current_energy == old_sam_energybar.current_energy + sam_vest.amount.value );
      BOOST_REQUIRE( db->get_account( "sam" ).downvote_energybar.current_energy == old_sam_downvote_energybar.current_energy + sam_vest.amount.value / 4 );
      BOOST_REQUIRE( db->get_account( "dave" ).voting_energybar.current_energy == old_dave_energybar.current_energy );
      BOOST_REQUIRE( db->get_account( "dave" ).downvote_energybar.current_energy == old_dave_downvote_energybar.current_energy );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_apply )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         db.modify( db.get_account( XGT_TREASURY_ACCOUNT ), [=]( account_object& a )
         {
      ...
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      XGT_REQUIRE_THROW( db->push_transaction( tx ), chain::plugin_exception );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - XGT_BLOCK_INTERVAL );

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [=]( dynamic_global_property_object& gpo )
         {
            gpo.current_supply -= gpo.total_reward_fund_xgt;
            gpo.total_reward_fund_xgt = ASSET( "100.000 TESTS" );
            gpo.current_supply += gpo.total_reward_fund_xgt;
         });
      });
      ...
      BOOST_REQUIRE( db->get_account( "bob" ).reward_vesting_xgt.amount + db->get_account( "bob" ).reward_sbd_balance.amount + db->get_account( "sam" ).reward_vesting_xgt.amount + db->get_account( "sam" ).reward_sbd_balance.amount + db->get_account( XGT_TREASURY_ACCOUNT ).sbd_balance.amount == db->get_comment( "alice", string( "test" ) ).beneficiary_payout_value.amount );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_sbd_balance.amount + db->get_account( "alice" ).reward_vesting_xgt.amount + db->get_account( XGT_TREASURY_ACCOUNT ).sbd_balance.amount ) == db->get_account( "bob" ).reward_vesting_xgt.amount + db->get_account( "bob" ).reward_sbd_balance.amount + 1 );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_sbd_balance.amount + db->get_account( "alice" ).reward_vesting_xgt.amount + db->get_account( XGT_TREASURY_ACCOUNT ).sbd_balance.amount ) == ( db->get_account( "sam" ).reward_vesting_xgt.amount + db->get_account( "sam" ).reward_sbd_balance.amount ) / 2 + 1 );
      BOOST_REQUIRE( db->get_account( "bob" ).reward_vesting_xgt.amount == db->get_account( "bob" ).reward_sbd_balance.amount + 1 );
      BOOST_REQUIRE( db->get_account( "sam" ).reward_vesting_xgt.amount == db->get_account( "sam" ).reward_sbd_balance.amount + 1 );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_validate )
{
   try
   {
      ...
      prop_op.owner = "alice";
      XGT_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when setting maximum_block_size below XGT_MIN_BLOCK_SIZE_LIMIT" );
      prop_op.props.erase( "account_creation_fee" );
      prop_op.props[ "maximum_block_size" ] = fc::raw::pack_to_vector( XGT_MIN_BLOCK_SIZE_LIMIT - 1 );
      XGT_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when setting sbd_interest_rate to XGT_100_PERCENT + 1" );
      prop_op.props[ "sbd_interest_rate" ].clear();
      prop_op.props[ "sbd_interest_rate" ] = fc::raw::pack_to_vector( XGT_100_PERCENT + 1 );
      XGT_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- failure when setting new sbd_exchange_rate with SBD / XGT" );
      ...
      uint64_t unit = uint64_t(1) << XGT_RD_DECAY_DENOM_SHIFT;

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one year" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / XGT_BLOCKS_PER_YEAR ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one day" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / XGT_BLOCKS_PER_DAY ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one hour" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / ((60*60)/XGT_BLOCK_INTERVAL) ) );
      prop_op.validate();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_apply )
{
   try
   {
      ...
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.maximum_block_size == XGT_MIN_BLOCK_SIZE_LIMIT + 1 );
      ...
      prop_op.props[ "key" ].clear();
      prop_op.props[ "key" ] = fc::raw::pack_to_vector( signing_key.get_public_key() );
      prop_op.props[ "account_subsidy_budget" ] = fc::raw::pack_to_vector( XGT_ACCOUNT_SUBSIDY_PRECISION );
      tx.clear();
      tx.operations.push_back( prop_op );
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.account_subsidy_budget == XGT_ACCOUNT_SUBSIDY_PRECISION );

      BOOST_TEST_MESSAGE( "--- Testing setting account subsidy pool cap" );
      uint64_t day_decay = ( uint64_t(1) << XGT_RD_DECAY_DENOM_SHIFT ) / XGT_BLOCKS_PER_DAY;
...
BOOST_AUTO_TEST_CASE( claim_account_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_account( XGT_NULL_ACCOUNT ).balance == ASSET( "5.000 TESTS" ) );
      ...
      BOOST_CHECK_EQUAL( c_subs, prev_c_subs - XGT_ACCOUNT_SUBSIDY_PRECISION );
      BOOST_CHECK_EQUAL( nc_subs, prev_nc_subs );
      ...
      int64_t new_value = prev_c_subs - XGT_ACCOUNT_SUBSIDY_PRECISION;     // Usage applied before decay
      new_value = new_value
          + 5000                                                             // Budget
          - ((new_value*249617279) >> XGT_RD_DECAY_DENOM_SHIFT);           // Decay
...
BOOST_AUTO_TEST_CASE( create_claimed_account_apply )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( XGT_TEMP_ACCOUNT ), [&]( account_object& a )
         {
...
BOOST_AUTO_TEST_CASE( account_auth_tests )
{
   try
   {
      ...
      vest( XGT_INIT_MINER_NAME, "alice" , ASSET( "10.000 TESTS" ) );
      vest( XGT_INIT_MINER_NAME, "bob" , ASSET( "10.000 TESTS" ) );
      vest( XGT_INIT_MINER_NAME, "charlie" , ASSET( "10.000 TESTS" ) );
      generate_block();
