...
BOOST_AUTO_TEST_CASE( undo_basic )
{
   try
   {
      ...
      const wallet_object& obj2 = ao.modify( obj1, [&]( wallet_object& obj ){ obj.name = "name01"; } );
      ...
      ao.modify( obj4, [&]( wallet_object& obj ){ obj.proxy = "proxy00"; } );
      ao.remove( obj4 );
      ...
      ao.modify( obj6, [&]( wallet_object& obj ){ obj.proxy = "proxy00"; } );
      ao.remove( obj6 );
      ao.create( [&]( wallet_object& obj ){ obj.name = "name00"; } );
      ...
      const wallet_object& obj_cm = ao.create( [&]( wallet_object& obj ){ obj.name = "name01"; } );
      BOOST_REQUIRE( std::string( obj_cm.name ) == "name01" );
      ao.modify( obj_cm, [&]( wallet_object& obj ){ obj.name = "name02"; } );
      BOOST_REQUIRE( std::string( obj_cm.name ) == "name02" );

      const wallet_object& obj_cr = ao.create( [&]( wallet_object& obj ){ obj.name = "name03"; } );
...
BOOST_AUTO_TEST_CASE( undo_different_indexes )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- 'wallet_object' - create/5*modify" );
...
BOOST_AUTO_TEST_CASE( undo_generate_blocks )
{
   try
   {
      ...
      _data data[4]=
      {
        _data( "bob", "post4", std::string(XGT_ROOT_POST_PARENT), "pl4", "t4", "b4" ),
        _data( "alice", "post", std::string(XGT_ROOT_POST_PARENT), "pl", "t", "b" ),
