...
tests = [
   ...
   },
   {
      ...
      "params": [{"operations":[["transfer", {"from":"temp", "to":"null", "ammount":"1.000 XGT"}]]}]
   },
...
#   },
   ...
   {
      "method": "database_api.list_xtt_token_emissions",
      "params": {"limit": 100, "order": "by_symbol_time", "start": []}
   },
   {  "method": "database_api.find_xtt_token_emissions",
      "params": {"asset_symbol": {"nai":"@@100000006", "decimals": 3}}
   }
   {
      "method": "database_api.list_xtt_contributions",
      "params": {"limit": 100, "order": "by_symbol_contributor", "start": []}
   },
   {
      "method": "database_api.find_xtt_contributions",
...
def main():
   if len( sys.argv ) == 1:
      url = "https://api.xgtit.com/"
   elif len( sys.argv ) == 2:
      url = sys.argv[1]
   else:
      exit( "Usage: api_error_smoketest.py <xgt_api_endpoint>" )
